...
public class GroupPooler : MonoBehaviour
{
    ...
    void Generate()
    {
        ...
        csv[(int)PlayTime].TryGetValue("GroupType", out object obj);
        // obj 가 없을때
        if (obj == "")
            return;
        ...
        PooledObject Group = Manager.Pool.GetPool(
        prefabs[(int)csv[(int)PlayTime]["GroupType"]],
        point[(int)csv[(int)PlayTime]["GroupYPos"]].position,
        point[(int)csv[(int)PlayTime]["GroupYPos"]].rotation);
    }
}
